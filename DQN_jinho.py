...
def main():
    ...
    for episode in range(EPISODE)[start:]:
        ...
        if episode % 10 == 0:
            ...
            #    if 'O' in c:
            #        O += 1
            #print("\njinho's acu: " + str(O * 100 / len(score_list)) + "%")
            print('\n문제 {0}개 중 {1}개 맞음!'.format(config.validate_num, right_count)) 
            print('\ntest result: ', score_list)
            print("\nMathDQN's answer: ", MathDQN_answer)
